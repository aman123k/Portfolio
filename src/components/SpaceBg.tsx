  ...
  useEffect(() => {
    ...
    const buildStars = () => {
      ...
      for (let i = 0; i < count; i++) {
        ...
        stars.push({
          ...
          vy: (0.04 + layer * 0.3) * (Math.random() > 0.5 ? 1 : -1) * 0.4,
    ...
    const buildGalaxy = () => {
      ...
      for (let i = 0; i < numParticles; i++) {
        ...
        if (randType < 0.2) {
          ...
          colorPrefix =
            Math.random() > 0.45 ? galaxyColorTemps[2] : galaxyColorTemps[3];
          ...
          const dispersion =
            (Math.random() - 0.5) * (0.24 + 40 / (distance + 10));
          angle = armAngle + dispersion;

          const colorRand = Math.random();
          if (colorRand < 0.35)
            colorPrefix = galaxyColorTemps[0]; // magenta/purple
          else if (colorRand < 0.7)
            colorPrefix = galaxyColorTemps[1]; // blue
          ...
          angle = Math.random() * 2 * Math.PI;
          colorPrefix =
            Math.random() > 0.5 ? galaxyColorTemps[1] : galaxyColorTemps[3];
        }
        ...
        galaxyStars.push({
          ...
          color: colorPrefix,
          opacity:
            (randType >= 0.85 ? 0.08 : 0.15) +
            (1 - distance / maxRadius) * 0.85,
        });
      }
    };
    ...
    const draw = (timestamp: number) => {
      ...
      for (let i = 0; i < numNebulae; i++) {
        ...
        const nebGrd = ctx.createRadialGradient(
          nebX,
          nebY,
          0,
          nebX,
      ...
      galaxyStars.forEach((gs) => {
        ...
        const angle =
          gs.angle + gs.distance * spiralFactor + t * baseRotationSpeed;
      ...
      shootingStars.forEach((ss) => {
        ...
        grad.addColorStop(
          0.6,
          `rgba(255,255,255,${(ss.opacity * 0.5).toFixed(3)})`,
        );
    ...
    return () => {
      ...
      [p2FloatRef, p3FloatRef].forEach(({ current }) => {
        if (current) gsap.killTweensOf(current);
      });
      mouseRefs.forEach(({ current }) => {
        if (current) gsap.killTweensOf(current);
      });
